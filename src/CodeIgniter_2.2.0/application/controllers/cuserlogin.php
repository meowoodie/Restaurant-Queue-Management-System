<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

/**
 * Controller Class : Cuserlogin 
 * 
 * Created on Friday. Aug 12. 2014
 *
 * @Author       : Woodie
 * @E-mail       : 304471139@qq.com
 * @Version      : 1.0
 * @UpdateLog    : None
 * @Description  : This Class is used to control whether or not users get into the system.0
 *                 It contains about Login and Registration features.
 * @RelatedView  : WelcomeResult.php at /views/mobile_app/
 * @RelatedModel : Model_User.php 
 * @RelatedLib   : None
 */

class Cuserlogin extends CI_Controller 
{
	/*************
	 * Constructor
	 */
	function __construct()
	{
		parent::__construct();
		// load in model( Model_user )
		$this->load->model("Model_User");
	}
	
	/******************
	 * Default Action :
	 * Print welcome message ..
	 */
	public function index()
	{
		$this->load->view('welcome_message');
	}
	
	/*******************
	 * Action : Register
	 * New user registration 
	 */
	public function Register()
	{
		// Obtain the info of Registration
		$user_name = $this->input->post('user_name');
		$password  = $this->input->post('password');
		$phone     = $this->input->post('phone');
		// It used to generate tmp user for manual
		/* if($user_name == "random")
		{
			$user_name = "临时用户" . '_' . date("YmdHis") . '_' . rand(1000, 9999);
		} */
		// Ensure your form's key fieled is not empty
		if($user_name == FALSE || $password == FALSE)
		{
			$register_info = array(
				'status'  => '2',
				'msg'     => 'Username, Pwd being empty is not allowed!'
				);
		}
		else
		{
			// Database operation : Check your username
			if($this->Model_User->checkUnique($user_name))
			{
				$this->Model_User->addUser($user_name,$password,$phone);
				$register_info = array(
					'status'  => '0',
					'msg'     => 'Register Successful!'
					);
			}
			else
				$register_info = array(
					'status'  => '1',
					'msg'     => 'Your username is exist!'
					);
		}
		// return login info 
		$this->load->view('mobile_app/welcome_result',$register_info);
	}
	
	/******************************
	 * Action : Register for manual
	 * New user registration 
	 */
	public function RegisterForManual()
	{
		// Obtain the info of Registration
		$user_name = $this->input->post('user_name');
		$password  = $this->input->post('password');
		$phone     = $this->input->post('phone');
		// It used to generate tmp user for manual
		if($user_name == "random")
		{
			$user_name = "临时用户" . '_' . date("YmdHis") . '_' . rand(1000, 9999);
		}
		// Ensure your form's key fieled is not empty
		if($user_name == FALSE || $password == FALSE)
		{
			$register_info = array(
				'status'  => '2',
				'msg'     => 'Username, Pwd being empty is not allowed!'
				);
		}
		else
		{
			// Database operation : Check your username
			if($this->Model_User->checkUnique($user_name))
			{
				$user_id = $this->Model_User->addUser($user_name,$password,$phone);
				$register_info = array(
					'status'  => '0',
					'msg'     => $user_id
					);
			}
			else
				$register_info = array(
					'status'  => '1',
					'msg'     => 'Your username is exist!'
					);
		}
		// return login info 
		$this->load->view('mobile_app/welcome_result',$register_info);
	}
	
	/*******************
	 * Action : Register
	 * Login the system 
	 */
	public function Login()
	{
		// Obtain the info of Login
		$user_name = $this->input->post('user_name');
		$password = $this->input->post('password');
		// Ensure your form is not empty
		if($user_name == FALSE || $password == FALSE)
		{
			$login_info = array(
				'status'  => '2',
				'msg'     => 'Username and Pwd being empty is not allowed!'
				);
		}
		else
		{
			// Database operation : Check your username and pwd
			if($this->Model_User->checkUser($user_name,$password))
			{
				$user_id = $this->Model_User->GetUserId($user_name);
				$login_info = array(
					'status'  => '0',
					'msg'     => $user_id
					);
			}
			else
			{
				$login_info = array(
					'status'  => '1',
					'msg'     => 'Login Failed!'
					);
			}
		}
		// return login info 
		$this->load->view('mobile_app/welcome_result',$login_info);
	}
}
?>
